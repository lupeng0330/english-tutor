...
- 🩹 **错题本**：答错自动收录、连对 2 次自动移除；阅读理解 / 课文自测错题在详情区直接显示文章原文，并可一键「去此单元重读」
...
- 📊 **学习报告**：图表展示学习时长、得分趋势
- 📱 **响应式设计**：完美适配手机 / 平板 / 电脑
- 📲 **PWA 可安装 + 离线可用**：支持"添加到主屏"，离线也能背单词（课文/题库本地缓存）
...
| 教材 | 单词拼写 | 听力 | 语法 | 阅读 | 合计 |
|------|---------|------|------|------|------|
| 广州教科版 (jk) | 224 | 52 | 72 | 72 | **420** |
| 沪教牛津版 (hj) | 698 | 32 | 480 | 288 | **1498** |
| **总计** | **922** | **84** | **552** | **360** | **1918** |

教材覆盖：**广州教科版 1-9 年级 83 单元** + **沪教牛津版 7-9 年级 48 单元**（每单元 15 词 + 3 篇课文）。
...
- [x] 1900+ 题真实题库（jk 420 + hj 1498；hj 语法/阅读 48 单元全覆盖）
...
- [x] 基于课文的 AI 自动生成题目
- [x] GitHub Pages 自动部署
- [x] **PWA 离线可用**（manifest + Service Worker；`data/*.json` SWR、`audio/*.mp3` cache-first；可"添加到主屏"离线背单词）
- [ ] 人教版 / 外研版教材补充
- [ ] 真实 AI 语音评测（接入云端 ASR）
- [ ] AI 对话接入 LLM
- [ ] 多用户账号 + 学习数据持久化
