...
> 最后更新：**2026-06-27 深夜**（本次：单词记忆曲线 SRS + 单词卡按钮重设计【方案B 侧翼圆钮+双主按钮，配色统一】，详见 §31 · 已用真实浏览器验证渲染、直接部署；前序 §30 gzk、§29 技术债、§28 在线本地化）
...
| 2026-06-26 晚 | **jk 教科版 7 册（3 上 → 6 上）全量补齐**：63 单元 / 490 词 / 126 课文 / 1437 例句 / 1344 新 ex_*.mp3 / 单册 verify 6 项全 PASS | §25（附录 A） |
| 2026-06-26 | jk 剩余 7 册补齐开发宪法 + 流水线模板化（`scripts/_jk_volume_lib.py` + `_verify_volume.py`）+ 铁律 5 入文档 | §23（附录 A） |
...
- [§0 给新进来的 AI 助手的一段话 + 五条铁律](#0-给新进来的-ai-助手的一段话)
- [§1 项目一句话简介](#1-项目一句话简介)
- [§2 技术栈与运行](#2-技术栈与运行)
...
1. 顶部 **🚀 速读卡**（项目 / 起服务 / 5 条铁律 / 最近大事 / 决策树 / 接手三件套）
2. **本节 §0**（5 条铁律完整版） + **§3 当前规模** + **§4 目录与关键文件** + **§5 重要约定（容易踩坑）**
...
## 3. 当前规模（2026-06-27 核查）

> 数据通过 `python -c "..."` 实测脚本核出，与磁盘实际文件一致；变动以本表为准，旧版表述请忽略。

### 教材

| 教材 | 状态 | 详情 |
|---|---|---|
| **教科版 jk** | ✅ 主力 | **82 个单元** —— 1-2 年级 8 单元占位（每年级×2 学期×2 占位）；🆕 **3 上 → 6 上 共 7 册 63 单元真实数据**（每册 9 单元 / 70 词 / 18 课文，2024 秋新版，详见附录 §22 / §24 / §25）；6 下 11 单元真实（2014 旧版，后续按需重做） |
| **沪教版 hj** | ✅ 完整 | **48 个单元**（7-9 年级 × 上下册 × 8 单元；每单元 15 词 + 3 篇课文 Reading / Grammar focus / More reading） |
| 广州口语 gzk | ⏳ 占位 | 1-2 年级 22 个单元 `placeholder:true` 空壳，**P1-5 待填充** |
...
|---|---|---|---|---|---|
| jk | 224 | 52 | 72 | 72 | **420** |
| hj | 698 | 32（7 上） | 480 | 288 | **1498** |

> hj 听力实际可用题量 192（含 hj_listening 全册），其中 7A 32 题为人工原题、其余 160 题由 P0-1（§24.6）AI 造题 + MP3 全量补全。

### 音频 MP3（合计 **约 4296 个**）

| 类别 | 数量 | 说明 |
|---|---|---|
| 教材课文 / 听力类 | ~218 + 245 = ~463 | 教科版课文 42 + 教科版听力 31 + 沪教听力 192 + 沪教课文 144 + 杂项 54 |
| 例句朗读 `ex_*.mp3` | **3833** | 沪教 6 册（约 1800）+ jk 6 下（约 493）+ 🆕 jk 3 上 → 6 上 7 册 1437 句（其中 1344 新生成、93 跨册 hash 复用，详见附录 §22 / §24 / §25） |
| 句级增量缓存 `audio/_sent/` | 不计入 | gitignore，单句重念缓存（gen_audio_v2.py 句级增量） |
| 音频增量 manifest | `audio/.manifest.json` | 143 条 hash 记录，零改动 0.53 秒扫完；句级模式下额外记录 `sentences[]` |
