...
> 当前优先级（2026-06-29 核查）：
> - ✅ **P0** 全清（听力 MP3 / sw.js / 小学考试配置 / 在线依赖兜底）
...
## 3. 当前规模（2026-06-29 核查）

> 数据通过 `python scripts/_verify_qbank.py` 实测 + 磁盘文件计数核出，与实际一致；变动以本表为准，旧版表述请忽略。
...
| 教材 | 拼写 | 听力 | 语法 | 阅读 | **完形（篇）** | 合计 |
|---|---|---|---|---|---|---|
| jk | 645 | 102 | 189 | 109 | **35（155 挖空）** | **1080** |
| hj | 698 | 192 | 480 | 288 | **36（180 挖空）** | **1694** |
| **gzk** | **244** | **46** | — | — | — | **290** |

> ✅ **完形填空（P2-C 批次 1 + 2 + 3 完成）**：hj 全 6 册 36 篇 / 180 挖空 + **jk 全 7 册 35 篇 / 155 挖空** = 总 **71 篇 / 335 挖空**。
> - hj：每篇 80-100 词 + 5 挖空（diff 2-4），每册 6 篇
> - jk：3-4 年级 40-60 词 + 4 挖空（diff 1-2），5-6 年级 60-90 词 + 5 挖空（diff 2-3），每册 5 篇 7 册 × 5 = 35 篇
> - 详见 §36（hj）+ §37（jk）。
...
| 状态 | 范畴 | 备注 |
|---|---|---|
| ✅ **P0 全清** | 听力 MP3 补全 / sw.js 缓存 / 小学考试配置 / 在线依赖兜底 / 例句朗读三级降级 | 详见 §21（附录 A） |
...
> **P0 / P1 / P2 全部清零** 🎉，后续主要是 P3 真功能接入或 P4 新教材建设。
...
| **P3-C** | 作文 AI 评分（替换 `exam.js` 启发式） | ~3-5 天 | 复用同 LLM Key |
| **P3-D** | 语法讲解页数据化（4 条硬编码 → 知识库 JSON） | ~3-5 天 | 纯数据填充 |
| **P4** | 人教 rj / 外研 wy 教材从 0 建设 | ~1 个月 | 复用 jk 7 册流水线 |
| **P5** | jk 6 下 重做为 2024 新版 | ~3 天 | 11 单元当前 2014 旧版升级到 2024 新版（注：jk 1-2 年级已澄清不存在，详见 §33） |
...
| jk 3 上→6 上 7 册 35 篇 / 155 挖空 | ✅ **完成（批次 3）** | 详见 §37；每册 5 篇，分级适龄设计 |
| **P2-C 全部合计** | ✅ **71 篇 / 335 挖空** | hj 36 + jk 35 |
...
- ✅ **批次 3 完成**：jk 7 册 35 篇 / 155 挖空（详见 §37）
...
## 37. ✅ P2-C 批次 3 · jk 小学完形填空（2026-06-30 上午）
...
| 每册篇数 | **5 篇/册** × 7 册 = **35 篇**（用户选 Q3=B 多 1 篇） | 4 篇高频主题 + 1 篇均衡选 |
...
| 文件 | 内容 | 题量 |
|---|---|---|
| `data/questions/jk_cloze.json`（新增）| 35 篇短文 / 155 挖空 | 3A/3B/4A/4B/5A/5B/6A 每册 5 篇 |

**难度分布**：diff1 = 10 篇 / diff2 = 14 篇 / diff3 = 11 篇

**全局规模变化**：
- 题库总数：2999 + 36 篇 cloze（hj）→ **2999 + 71 篇 cloze**（+35 篇 / +155 挖空）
- jk 总数：1045 → **1080**（+35 篇）
- hj + jk 完形总篇数：36 → **71 篇**（hj 36 + jk 35），总挖空 180 → **335**
...
| 项 | 结果 |
|---|---|
| `build_cloze.py` schema 预校验 | 35/35 通过 |
| `_verify_qbank.py` 退出码 | 0（3064 题全 PASS，3029→3064 +35）|
...
| 8 | F12 控制台 | `[题库] 加载完成 (jk): 单词 645 · 听力 102 · 语法 189 · 阅读 109 · 完形 35 · 共 1080 题` |

### 37.7 收口

- ✅ **P2-C 全部完成**（三批次：工程基建 + hj 36 篇 + jk 35 篇 = 71 篇 / 335 挖空）
- ✅ jk 小学 3 上→6 上 全 7 册完形覆盖（35 篇覆盖每册 5 个核心单元话题）
...
- ✅ **P2-C 三批次累计**：V02.44（批次 1 工程基建 + hj 7A）→ V02.45（批次 2 hj 7B-9B）→ V02.46（批次 3 jk 全 7 册），共 71 篇 / 335 挖空 / 13 个学期完形覆盖。

---

## 38. ✅ 小学模拟考试 · 阶段 1（100 分制 + 期中 + 完形修复，2026-06-30 中午）

> 📌 触发：用户在 P2-C 验收完成后反馈两个问题：① 小学单元测试卷只有 40 多分（与广州真实考试 100 分差距大）② 小学模拟考试看不到完形大题（P2-C 已造 35 篇 jk 完形但配置未引用）。多信源调研后得出新方案 v2（详见 artifact），分 4 阶段推进，本节为**阶段 1（配置改造）**收口。
...
| 小学完形 | **完全没有 cloze section**，jk_cloze.json 35 篇数据闲置 |
...
**③ 补 jk 6 下 5 篇 cloze**（Q5A）：扩 `scripts/jk/build_cloze.py` 加 `jk_6b()` 函数 5 篇：U1 龟兔赛跑 / U2 守株待兔 / U4 保护大象 / U5 孙中山 / U8 礼貌用语；jk_cloze.json 35 → **40 篇 / 180 挖空**。
...
### 40.8 下一档预告（B2 · 已完成待验收 / B3 · 高段题库扩容 / B4 · 验收回滚预案）

- **B2**：✅ 已完成，待用户双端验收；本次按 C 方案同时扩低段通用题库 + 新增广东小学仿真真题（详见 §41）
...
## 41. B2-C · 低段题库扩容 + 广东小学仿真真题（2026-07-02，待验收）
...
- ⏳ 待用户双端验收通过后：执行 `dev-push.ps1` 自动上线，并将本节状态更新为“已上线（含版本号）”。
