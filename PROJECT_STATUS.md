...
开工前：
  ...
  ▼ P6-E 16套模板 已验收 ✅ → 双端出卷正常/全100分(§55.7)
  ▼ QZ-R 小学7册理解自测原文级重做 已上线 ✅ → C方案，答案可在课文定位(§56.6, V03.19)
  ▼ HJ-R 初中6册理解自测优化 已上线 ✅ → 每篇6选+4问+lesson.js匹配修复(§57, V03.21)
  ▼ 下一站 → 6下理解自测对齐 or 三期 P4（人教 rj / 外研 wy 新教材建设，~1月）
  ▼ 三期完成 + 有 Key？ → 做 P3-A/B/C（AI 对话 / ASR / 作文评分）
  ▼ 都完成？           → 维护：跑 tests/smoke.py 回归 / 升级依赖 / 清理技术债
```

> 当前优先级（2026-07-09 进度对账更新，铁律 2/5）：
...
> - ✅ **QZ-R**：小学全 7 册理解自测原文级重做（§56.6，C方案，V03.19 已上线）
> - ✅ **HJ-R**：初中全 6 册理解自测优化 + `lesson.js` 匹配Bug修复（§57，V03.21 已上线）
> - 🎯 **下一站二选一**：①6下理解自测对齐（收尾，1册）②三期 P4 人教 rj + 外研 wy 新教材建设（~1 月，无需 Key）
...
- [§1 项目一句话简介](#1-项目一句话简介)
- [§2 技术栈与运行](#2-技术栈与运行)
- [§3 当前规模（2026-07-05 核查）](#3-当前规模2026-07-05-对账核查)
- [§4 目录与关键文件](#4-目录与关键文件)
- [§5 重要约定（容易踩坑）](#5-重要约定容易踩坑)
- [§6 部署与发布](#6-部署与发布)
- [§7 近期进展（Git log 摘要）](#7-近期进展git-log-摘要)
- [§8 后续版本开发计划](#8-后续版本开发计划)
- [§9 常见命令速查](#9-常见命令速查)

### 当前活跃计划（重点看这几节）

- [§52 P6 最终开发计划（权威）](#52--p6-最终开发计划代码核查修正版2026-07-04)
- [§53 P6-C 写作 + P6-E 16套模板（已验收）](#53--p6-c-写作--p6-e-16套模板完成2026-07-04-完成--07-05-验收通过已上线)
- [§54 T1-T4 6套主题系统 + contextBar 修复](#54--t1-t4--6-套主题系统--contextbar-下拉框修复2026-07-04--07-05已上线)
- [§55 进度对账 · P3-D + P6 全系已完成](#55--进度对账补录--p3-d--p6-ab-d-已完成2026-07-05-核查)

### 附录 A · 完成记录档案（§10-§50，按需检索）

> 历史完成记录 **§10-§50** 均已标 `[已归档]` 前缀，**不是当前活跃任务**，新 AI 接手无需逐章读完；查具体历史决策 / 实现细节再按需翻阅。下面是 §10-§25 的时间倒序索引（§26-§50 为 06-27~07-04 的内容建设/考试改造记录，同样已归档）：
...
> 📌 最近一次文档状态校准：2026-07-05（**重度精简：§26-§50 全部标 [已归档]，正文仅留速读卡+§0铁律+§3规模+§52-55当前活跃计划**；P6全系+P3-D+T主题 已上线）
...
### 题库（合计约 **3600 题/组/篇** · 2026-07-05 实盘）

**基础题型**

| 教材 | 拼写 | 听力 | 语法 | 阅读 | 完形(篇) |
|---|---|---|---|---|---|
| **jk** | 645 | 125 | 235 | 160 | 56 |
| **hj** | 698 | 192 | 480 | 288 | 36 |
| **gzk** | 244 | 46 | — | — | — |

**P6 新题型（jk 教材，全链路：考试+练习+智能推题+错题本）**
...
| 听音判断 | 50 | 句型转换 | 40 |
| 听音选图 | 50 | 匹配题 | 10 组 |
| 补全短文 | 9 篇 | 补全对话 | jk 17 + hj 8 |
| 书面表达 | 16 篇 | 语法知识库 | 80 条 |
...
> - 🆕 **理解自测（QZ-R §56.6 + HJ-R §57，原文级重做已上线 V03.21）**：`data/extras/*_exercises.json` 共 14 册，合计 **1302 选择 + 879 问答 + 64 语法表**。小学 jk 7 册（3上~6上）按 C 方案原文级重做——每题答案可在 `textbooks/jk.json` 对应课 `en` 原文定位；3-4年级每篇3选择+9语法表/册，5-6年级每篇(GS+RR)3选+2问=54选+36问+9语法表/册。初中 hj 6 册（7上~9下）每篇课文6选+4问=10题/篇，每册144选+96问。`js/lesson.js` 已修复 `·` 分隔符匹配 + page 精确相等。**6下为历史全量版未对齐（60选+195问+1语法表），待后续处理。**
...
## 26. [已归档] ✅ 课文朗读情感韵律增强 + 全年级多角色男女声重生成（2026-06-27 晚 · 待验收）
...
## 27. [已归档] ✅ 单词卡发音本地 MP3 化 · 根治"认识不发音"（2026-06-27 晚 · 待验收）
...
## 28. [已归档] ✅ P0-2 在线依赖排查 + 本地化兜底（2026-06-27 晚 · 已验收）
...
## 29. [已归档] ✅ 技术债清理 T-1/T-2/T-3（2026-06-27 晚 · 待验收）
...
## 30. [已归档] ✅ P0-1 广州口语 gzk 填充 · 批次1 grade1上（2026-06-27 晚）
...
## 31. [已归档] ✅ jk 小学 1-6 年级全册题库补齐（2026-06-28 晚 · 已上线）
...
## 33. [已归档] ✅ 澄清 jk vs gzk 教材边界 · jk 三年级起点（2026-06-29）
...
## 34. [已归档] ✅ P2-B · gzk 题库补齐（2026-06-29 下午）
...
## 35. [已归档] ✅ P2-B 质量增强 + 全局 Bug 修复（2026-06-29 下午）
...
## 36. [已归档] ⏳ P2-C 完形填空独立题库（批次 1 工程基建 + hj 7A · 批次 2 hj 7B/8A/8B/9A/9B；2026-06-29 深夜）
...
## 37. [已归档] ✅ P2-C 批次 3 · jk 小学完形填空（2026-06-30 上午，实盘 56 篇）
...
## 38. [已归档] ✅ 小学模拟考试 · 阶段 1（100 分制 + 期中 + 完形修复，2026-06-30 中午）
...
## 39. [已归档] ✅ 三大题型重做 + 听音填空 bug 修复 + 新听力三题型 MP3（2026-07-01 下午）
...
## 40. [已归档] ✅ A 档 B1 · 考试配置模板化（2026-07-02 用户双端验收通过）
...
## 41. [已归档] B2-C · 低段题库扩容 + 广东小学仿真真题（2026-07-02，已上线 `20260702V03.04`）
...
## 43. [已归档] 📋 开发计划重构 · 逐期推进（2026-07-03 深夜）
...
## 44. [已归档] 🚧 P5 · jk 6 下内容审查 + U11 Review 中国化替换（方案制定，待执行）
...
## 45. [已归档] ⚠️ 审查纠错 · 全年级内容审查报告（2026-07-03 深夜）
...
## 46. [已归档] ✅ P5 一期 · U11 Review 中国化替换完成（2026-07-03 深夜，已上线）
...
## 47. [已归档] 📋 P3-D · 语法讲解页数据化完整方案（2026-07-03 深夜制定）
...
## 48. [已归档] 📋 广州小学英语考试题型对账分析报告（2026-07-04 修订版）
...
## 49. [已归档] 📋 各年级/学期题型详细对比（2026-07-04 补充分析）
...
## 50. [已归档] 📋 P6 · 小学模拟考试题型对齐广州官方 · 开发计划（2026-07-04）
...
## 51. [已删除] P6 完整开发计划（DeepSeek 初稿）

> 本节 228 行废弃初稿已于 2026-07-05 删除（重度精简）。**权威计划见 §52（代码核查修正版）**，实际完成情况见 §53/§55。
...
## 53. ✅ P6-C 写作 + P6-E 16套模板完成（2026-07-04 完成 / 07-05 验收通过，已上线）
...
### 55.7 P6-E 双端验收通过（2026-07-05）

> 用户双端验收通过（铁律 1）。本地 `python -m http.server 8000` 双端 URL 实测。

**静态校验（代码级）**：

| 项 | 结果 |
|---|---|
| 16 套模板齐全 | ✅ g{3-6}{a/b}_{midterm/final} |
| 每套总分=100 | ✅ 16/16 全部 `count×points` 合计=100.0（=声明 totalPoints） |
| exam_config 指向 | ✅ 3-6 年级期中/期末指向正确 |
| section 题库非空 | ✅ 16 套所有题型题库均有货（无空跳过） |

**题库实盘**：听力125/拼写645/语法235/阅读160/完形56/匹配10/连词成句73/补全对话17/句型转换40/补全短文9/写作16

**⚠️ 澄清历史误判**：§53.3 曾疑 `points=0` bug——实为字段名是 `points`，exam.js 第201行按 `count × points` 计算总分；粗算若误取单题分会得 22-35 假象，正确公式下 16 套全=100。

**双端**：PC `index.html` + 手机 `mobile.html` 出卷正常，新题型（补全对话/匹配/补全短文/句型转换）渲染作答正常，判分闭合。

> 本节 docs 收口，CI 不 bump。

---

## 56. 🎯 QZ · 理解自测补全计划（2026-07-06 启动）

> 📌 **背景**：jk 教科版 3-6 年级课文页下方「理解自测」功能前端全链路已完成（`js/lesson.js`：加载→渲染→提交判分→错题本→进度统计），但数据层 `data/extras/` 仅覆盖 6 下 1 册。本次为 3 上~6 上 7 册 126 篇课文补齐全量自测题 + 语法句型表。

### 56.1 出题规范（用户已确认）

| 参数 | 3-4 年级（低段） | 5-6 年级（高段） |
|---|---|---|
| 每篇题量 | 3 题（全选择题 `choice_qa`） | 5 题（3 选择 `choice_qa` + 2 问答 `reading_qa`）|
| 答案来源 | 课文原文直接可定位（一句话抓取） | 课文原文 + 简单推断 |
| 干扰项 | 3 选 1，干扰项来自同课单词/话题 | 3 选 1，干扰项适当增加迷惑性 |
| grammar_table | ✅ 每单元 1 组语法句型对照表 | ✅ 每单元 1 组语法句型对照表 |

### 56.2 分册交付顺序

| 批次 | 册 | 单元 | 课文 | 选择 | 问答 | 总题 | grammar_table |
|---|---|---|---|---|---|---|---|
| QZ-1 | 3 上 | 9 | 18 | 54 | 0 | 54 | 9 组 |
| QZ-2 | 3 下 | 9 | 18 | 54 | 0 | 54 | 9 组 |
| QZ-3 | 4 上 | 9 | 18 | 54 | 0 | 54 | 9 组 |
| QZ-4 | 4 下 | 9 | 18 | 54 | 0 | 54 | 9 组 |
| QZ-5 | 5 上 | 9 | 18 | 54 | 36 | 90 | 9 组 |
| QZ-6 | 5 下 | 9 | 18 | 54 | 36 | 90 | 9 组 |
| QZ-7 | 6 上 | 9 | 18 | 54 | 36 | 90 | 9 组 |
| **合计** | **7 册** | **63** | **126** | **378** | **108** | **486** | **63 组** |

> 每批：做 JSON 数据 → 双端验证 → 验收通过 → `dev-push.ps1` 上线 → 下一批。

### 56.3 数据格式（与现有 `jk_grade6_xia_exercises.json` 对齐）
...
```json
{
  "exercises": {
    "u1": [
      {
        "kind": "choice_qa",
        "title": "Get started | Letters around us",
        "data": [
          { "题号": 1, "问题": "...", "选项": ["A. ...", "B. ...", "C. ..."], "答案": "A" },
          { "题号": 2, "问题": "...", "选项": ["A. ...", "B. ...", "C. ..."], "答案": "B" },
          { "题号": 3, "问题": "...", "选项": ["A. ...", "B. ...", "C. ..."], "答案": "C" }
        ]
      },
      {
        "kind": "reading_qa",
        "title": "Reading Room | The ABC song",
        "data": [
          { "问题": "...", "答案": "..." },
          { "问题": "...", "答案": "..." }
        ]
      },
      {
        ...
        "data": [
          { "句型": "This is a/an ...", "例句": "This is a bus.", "提示": "不定冠词 a/an 用法" }
        ]
      }
    ]
  }
...
- `title` 格式统一为 `"{page} | {lesson.title}"`，确保与 `_findExerciseForLesson()` 关键词匹配（支持 page/title 子串命中）。
- `choice_qa` 数据块含 `题号`/`问题`/`选项`/`答案`；`reading_qa` 含 `问题`/`答案`。
- `grammar_table` 不转自测题，在课文页下方折叠展示为句型参考表。

### 56.4 验收标准（每批）

| # | 验收项 | 操作 |
|---|---|---|
| 1 | 自测题出现 | 切到对应年级/学期 → 进入任一单元 → 课文页 → 下方出现「理解自测」区 |
| 2 | 选择题答题 | 点击选项 → 选中态正确 → 展开/收起正常 |
| 3 | 问答题答题（5-6 年级） | 英文输入 → 文本框自适应高度 |
| 4 | 提交判分 | 点「提交」→ 答对/答错逐题标注 + 总分提示 |
| 5 | 错题入本 | 答错题 → 错题本 reading_qa Tab 可见 |
| 6 | 语法表展示 | 每单元 grammar_table 折叠在课文下方 |
| 7 | 进度联动 | 答对题 → 首页统计 / 单元进度条更新 |
| 8 | 手机端 | `mobile.html` 重复上述关键路径 |

### 56.5 状态追踪

| 批次 | 内容 | 状态 |
|---|---|---|
| QZ-1 | 3 上 18 篇 + 9 语法表 | ✅ 已上线 `20260706V03.16` |
| QZ-2 | 3 下 18 篇 + 9 语法表 | ✅ 已上线 `20260707V03.17` |
| QZ-3 | 4 上 18 篇 + 9 语法表 | ✅ 已上线 `20260707V03.18` |
| QZ-4 | 4 下 18 篇 + 9 语法表 | ✅ 已上线 `20260707V03.18` |
| QZ-5 | 5 上 18 篇 + 9 语法表 | ✅ 已上线 `20260707V03.18` |
| QZ-6 | 5 下 18 篇 + 9 语法表 | ✅ 已上线 `20260707V03.18` |
| QZ-7 | 6 上 18 篇 + 9 语法表 | ✅ 已上线 `20260707V03.18` |

### 56.6 🔴 QZ-R · C 方案原文级重做（2026-07-07）

> 📌 **背景**：QZ-1~7 上线后发现**题文与课文不匹配**——题目问的内容在对应课 `en` 原文里根本无法定位（例：3 上 U1 课文是字母表 *The ABC song*，题却问 breakfast）。工作区 DeepSeek 的「完形挖空」修复版用户不认可（已 `git stash` 封存备用）。用户拍板 **S1+B1+F1**：全 7 册按「课文原文级」重做，逐册验收上线，从 4 上开工。

**C 方案铁律**：每题的问题 + 答案必须能在 `data/textbooks/jk.json` 对应课 `en` 原文里**指着句子找到**；干扰项有迷惑性但明确错；`grammar_table` 全部保留原有（本就贴合主题）；不改前端 `lesson.js`。

**出题量（用户二次确认）**：
- 3-4 年级（低段）：每篇课文 3 选择题；每册 = 18 篇 × 3 = **54 选择 + 9 语法表**。
- 5-6 年级（高段）：**每篇课文（Get started + Reading Room）都是 3 选择 + 2 问答**；每册 = 每单元 (6 选择 + 4 问答) × 9 = **54 选择 + 36 问答 + 9 语法表**。

| 批次 | 册 | 题量 | 状态 |
|---|---|---|---|
| QZ-R1 | 4 上 | 54 选择 + 9 语法表 | ✅ 已上线 `20260707V03.18`（`8a10930`） |
| QZ-R2 | 4 下 | 54 选择 + 9 语法表 | ✅ 已上线（C 方案统一批） |
| QZ-R3 | 5 上 | 54 选择 + 36 问答 + 9 语法表 | ✅ 已上线（C 方案统一批） |
| QZ-R4 | 5 下 | 54 选择 + 36 问答 + 9 语法表 | ✅ 已上线（C 方案统一批） |
| QZ-R5 | 6 上 | 54 选择 + 36 问答 + 9 语法表 | ✅ 已上线（C 方案统一批） |
| QZ-R6 | 3 上 | 54 选择 + 9 语法表 | ✅ 已上线（C 方案统一批） |
| QZ-R7 | 3 下 | 54 选择 + 9 语法表 | ✅ 已上线（C 方案统一批） |

> 备注：6 下（`jk_grade6_xia_exercises.json`）为历史全量版，本次未动。DeepSeek 完形挖空版仍存于 `git stash`，未采用。

---

## 57. 🔴 HJ-R · 初中理解自测优化（2026-07-08）

> 📌 **背景**：初中（沪教牛津·广州 2026）理解自测题存在两个问题——①**题量偏大**（普遍 17~26 题/单元）；②**比例失衡**（问答远多于选择，约 3:7，与需求相反）。用户要求：每篇课文压到 **10 题左右**、**选择:问答 ≈ 6:4**，并检查题目与课文关联性。

**用户拍板方案**：`Q1=B + Q2=A + Q3=A + Q4=A`

- **Q1=B**：Grammar focus 篇**保留并补题**（原计划 A 删除，用户改为 B 保留），基于该篇语法课文出题。
- **Q2=A**：每篇课文 **6 选择 + 4 问答 = 10 题**（正好 6:4）。
- **Q3=A**：全 6 册（7上~9下）统一重做，逐册/统一验收上线。
- **Q4=A**：全部以课文原文为依据重出，答案可在原文定位。

**结构**：初中每单元 3 篇课文（Reading / Grammar focus / More reading），每篇 6 选 + 4 问 → 每单元 30 题（18 选 + 12 问）；每册 8 单元 = **144 选择 + 96 问答**。

**关键 Bug 修复**（`js/lesson.js`）：`_findExerciseForLesson()` 原只认 `|` 分隔符（小学），初中题块用 `·`（U+00B7）分隔导致 page 前缀未拆出、第一层过滤失效，`"reading"` 误命中 `"more reading"`，Reading 篇吞掉 More reading 篇的题。修复：分隔符同时识别 `|` 与 `·`，page 过滤由「子串包含」改为「精确相等 `===`」。小学 page（Get started / Reading Room / Wonderland）不受影响。

| 批次 | 册 | 题量 | 状态 |
|---|---|---|---|
| HJ-R1 | 7 上 | 144 选 + 96 问 | ✅ 已上线（初中优化统一批） |
| HJ-R2 | 7 下 | 144 选 + 96 问 | ✅ 已上线（初中优化统一批） |
| HJ-R3 | 8 上 | 144 选 + 96 问 | ✅ 已上线（初中优化统一批） |
| HJ-R4 | 8 下 | 144 选 + 96 问 | ✅ 已上线（初中优化统一批） |
| HJ-R5 | 9 上 | 144 选 + 96 问 | ✅ 已上线（初中优化统一批） |
...
| 掌握度 | 各项计数取大 |
| SRS | 同词取 last 较新者 |
| 主题/智能推题 | 时间戳信封 Last-Writer-Wins（最终修复见 §60.1） |
| 学习上下文 | `__ts` 时间戳 Last-Writer-Wins |
...
## 61. ✅ Phase 3.5 离线语音包 · 按册下载 + 自包含学习数据（2026-07-30，已验收上线 `20260730V03.27`）

> 📌 **目标**：Web/PWA 用户主动按教材册下载，断网后仍可进入对应教材并播放已有 MP3；Capacitor 移动端继续把全部音频随安装包内置。电脑网页与手机布局已由用户验收通过。

### 61.1 离线包范围

- 自动生成 **18 册**包：广州口语 1-2 年级、广州教科版 3-6 年级、广州沪教版 7-9 年级，各分上下册。
- 全量覆盖 **5581 个去重 MP3（约 118.4 MB）**；共享单词/例句只存一份，不因下载多册重复占空间。
- 每册同时包含对应教材、该教材全部题库、当前册例句/扩展练习、语法库、考试配置/模板、历年真题索引，共 **61 个去重数据文件**；非默认教材下载后也可断网进入。
- `scripts/build_offline_audio_manifest.py` 从真实教材/题库引用生成 `data/offline-audio-packs.json`（schema v2），缺失显式音频、空包或未登记 MP3 会直接构建失败；临时文件校验后原子替换正式清单。

### 61.2 下载、更新与存储管理

- 个人中心新增「离线语音包」：显示当前册状态、浏览器存储占用、18 册列表、当前教材高亮。
- 支持 4 路并发、单文件 20 秒超时、最多 3 次重试、失败后继续下载；下载过程中每 10 个资源持久化一次进度。
- `lexue-audio-offline-v1` 存 MP3，`lexue-data-offline-v1` 存教材学习数据；两者跨应用版本保留。
- 清单给每个资源和每册生成内容修订号；更新时只重新请求缺失或修订变化的资源，杜绝同名旧 MP3/JSON 被误判为最新版。
- 可按册删除或全部删除；共享资源在其他完整/部分包仍使用时受保护，同时清理历史 `lexue-runtime-*` 音频占用。
- Capacitor 原生端显示「随包可用」，不重复下载；`mobile/scripts/copy-web-assets.js` 已实测把模块、清单和音频同步进 `mobile/www`。

### 61.3 Service Worker 离线壳加固

- 关键应用壳 + 默认教科版教材/核心题库采用原子预缓存；关键资源任一失败则拒绝安装并保留上一版。
- 安装请求统一携带应用版本并使用 `cache: reload`，防止浏览器 HTTP 旧缓存混入新版本；不再 `skipWaiting` 中途接管旧页面。
- `data/*.json` 优先查用户下载的数据包，音频优先查用户下载的音频包；`offline-download` 更新请求严格直通网络，避免假更新。
- 修复原 SW `staleWhileRevalidate` 把 Promise 当 Response 返回、带 `?v=` 离线不命中、断网激活误删应用壳等问题。
- 本地/局域网联网预览仍注销 SW、清版本壳，但保留用户主动下载的两个离线包缓存；本地断网重载时不清缓存，便于验收。

### 61.4 自动化实测

| 场景 | 结果 |
|---|---|
| 管理器打开 | 18 册完整显示，当前册高亮 |
| 广州口语一年级上册下载 | 102 音频 + 7 学习数据全部缓存，修订记录完整 |
| 断点续传 | 删除单文件后继续下载，只补齐缺失资源 |
| 更新绕缓存 | 数据缓存注入旧哨兵后，更新请求取得网络真实教材，不被旧缓存截获 |
| 共享文件保护 | 下载上下册后删除上册，下册共享音频仍完整 |
| 真断网请求 | 广州口语教材、题库、真题索引、课文 MP3 均 HTTP 200 |
| 真断网整页重载 | 应用壳、动态脚本、默认教材完整启动，无「资源加载不完整」 |
| 清理 | 自动化下载、元数据和测试 SW 已全部移除，不留测试状态 |

### 61.5 涉及文件

- 新增：`js/offline-audio.js`、`scripts/build_offline_audio_manifest.py`、`data/offline-audio-packs.json`。
- 修改：`app.js`（个人中心入口）、`index.html`（模块加载/SW版本注册/本地缓存策略）、`styles.css`（电脑+手机管理器）、`sw.js`（离线壳与稳定资源缓存）。

### 61.6 验收与上线

- ✅ 2026-07-30：用户确认电脑网页与手机布局验收通过。
- ✅ 已按铁律执行 `dev-push.ps1`，上线版本：`20260730V03.27`。
- 🌐 线上地址：<https://lupeng0330.github.io/english-tutor/?v=20260730V03.27>。

---

## 62. ✅ 智能推荐练习 + 错题本题目完整性修复（2026-07-30，已验收上线 `20260730V03.28`）

### 62.1 用户反馈与根因

- 首页智能推荐抽到「请点击单词，组成正确句子」时没有词块，无法作答。
- 根因是智能推荐把 14 种原始题型直接混合交给只完整支持部分题型的练习渲染器；`sentence_order` 只有 `words` 和字符串答案，却被当成普通 `options` 单选题。
- 同类审计还发现：`blank_fill`、`listen_judge`、整篇 `cloze` 无专用练习交互；`listen_pic` / `listen_fill` 无播放入口；广州口语听力使用 A-D 答案但通用判分要求数字；单字母拼写没有输入格。
- 错题本直接保存各模块原始对象，题干、答案、ID 与重练分别猜字段；结构化题型会显示无题干/答案横线、无法重练或同 code 互相覆盖。考试完形还会让整篇所有空共用一个错题 ID，答对空可能抵消答错空。

### 62.2 修复内容

- 新增统一题目规范化与可作答校验：字母答案转索引、判断题转二选一、历史文本错题兼容、整篇完形拆为逐空题；无有效题干/答案/控件的损坏数据不进入练习。
- 为连词成句增加词块选择/撤回/清空/提交，为完成句子增加中文题干+逐空选词，为课文自测/不规则动词历史错题增加文本输入重练。
- 听音选图、听音判断、听音填空统一显示播放入口；单字母拼写强制生成输入格。
- 错题 ID 纳入 `statement/passage/original/words/blanks/dialogue/pairs` 等结构化字段，并保留历史 key 以便旧错题答对后正常移出。
- 错题列表统一提取 16 类题目的可读题干与答案；考试完形按当前空保存，错题与掌握度共用同一快照。

### 62.3 验证结果

- 当前广州教科版 14 类智能推荐题型全量扫描：**0 无效题、0 重复 ID**；完形 56 篇正确展开为 244 道逐空题。
- 广州口语 + 沪教版共 8 组题库扫描：**0 无效题、0 重复 ID、0 未转换字母答案**。
- 连续生成 30 轮真实智能推荐共 300 题：每题均有题干与作答控件，0 失败。
- 真实提交连词成句、完成句子、听音判断均正确判分；错题本写入/列表/重练覆盖 16 类记录，0 无题干、0 答案横线、0 无控件。
- 390×844 手机布局：答题卡宽 342px，无横向溢出；项目既有 smoke：35 函数 + 4 命名空间齐全，0 JS 错误、0 断言失败。

### 62.4 涉及文件

- `js/practice.js`：统一题目模型、智能推荐建池、专用渲染/判分、听力入口与单字母拼写。
- `js/wrongbook.js`：数据校验、稳定 ID、题干/答案兼容、历史 key 重练。
- `js/exam.js`：完形错题按空保存，并与掌握度统一题目快照。

### 62.5 追加修复：智能推荐拼写题弹出 `undefined`（2026-07-30）

- **现象**：智能推荐做到单词默写时，若初始化或年级/学期/教材上下文刷新，弹出“当前学段下暂无 `undefined` 题目”并退出答题。
- **根因**：混合推荐把 `state.quizType` 标记为虚拟类型 `smart`，但上下文刷新仍执行 `filterQuestions('smart')`；题库没有 `smart` 数组，随后空题提示的旧题型映射也没有 `smart` 标签。
- **修复**：上下文刷新遇到 `smart` 时重新执行跨题型智能组题，不再把它当真实题型查询；空题提示补齐全部 14 类真实题型并增加“当前类型”兜底，禁止再显示 `undefined`。
- **实测**：智能推荐拼写题中直接触发刷新 → 保持 10 题、答题页可见、0 弹窗；三年级上册切换到下册 → 10 题全部属于下册、toast 显示“三年级下册 · 智能推荐”、0 弹窗。

### 62.6 验收与上线

- ✅ 2026-07-30：用户确认电脑网页与手机布局验收通过。
- ✅ 已按铁律执行 `dev-push.ps1`，上线版本：`20260730V03.28`。
- 🌐 线上地址：<https://lupeng0330.github.io/english-tutor/?v=20260730V03.28>。

---

## 63. ✅ 智能推荐 Ben 学科听力题与录音生命周期修复（2026-07-30，已验收上线 `20260730V03.29`）

### 63.1 用户反馈与根因

- **反馈**：智能推荐中的 “What subject does Ben find difficult?” 题干与听力录音对不上。
- **内容根因**：题库写成 Ben 认为 `Science` 困难，但五年级上册 U2 教材原文明确为 “I like science and PE. Maths is a little difficult.”。
- **生成根因**：78 道教科版听力把 `M:` / `W:` 对话写在同一行，而生成器只按换行识别角色，导致整段由首个角色朗读，并可能把后续角色标记当正文。
- **播放根因**：答题时点击下一题没有停止当前 MP3/TTS，旧录音可能继续覆盖新题页面，造成看似“题干与录音不一致”。

### 63.2 修复内容

- 将目标题统一为教材标准答案 `Maths`，同步修正听力原文、选项、答案、解析和题库构建源。
- 听力生成器现在会先把行内 `M:` / `W:` 拆行，再按角色分配男女声；新增 `--force`，允许原文修正后安全覆盖已有 MP3。
- 审计并重生成五年级上册 `jk_listening_5A_01.mp3` 至 `jk_listening_5A_10.mp3`，每段均按角色正确切分。
- 开始练习、智能推荐重新组题、点击下一题时统一停止并释放旧 MP3/TTS。
- 重建离线包清单：问题音频 revision=`6fb36625a584c168`，五年级上册包 revision=`da2c68c54cd47a4b`；网页与 Capacitor 资源哈希一致。

### 63.3 验证结果

- Ben 题页面实际数据：题干、原文、正确选项 `Maths`、解析完全一致；MP3 可解码，时长 12.22 秒。
- 五年级上册 10 道听力逐题审核正确答案映射，并由浏览器逐个加载 MP3：10/10 成功，时长 11.52–17.69 秒。
- Ben 题播放中点击下一题：依次执行 `pause → remove src → load`，旧录音不再延续到下一题。
- 新录音角色切分实测：`M→Ryan`、`W→Emma`、`M→Ryan`；浏览器 0 JS 错误；项目 smoke 0 错误、0 断言失败。

### 63.4 涉及文件

- `data/questions/jk_listening.json`、`scripts/jk/build_qbank.py`：修正 Ben 题内容与单一构建源。
- `audio/jk_listening_5A_01.mp3`–`jk_listening_5A_10.mp3`：五年级上册听力重新生成。
- `scripts/gen_listening_audio.py`：行内角色切分与 `--force`。
- `js/practice.js`：换题/重组题停止旧音频。
- `data/offline-audio-packs.json`、`mobile/www/**`：离线 revision 与手机资源同步。
